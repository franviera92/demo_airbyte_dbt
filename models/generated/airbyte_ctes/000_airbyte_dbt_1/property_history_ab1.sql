{{ config(
    ...
    schema = "_airbyte_000_airbyte_dbt_1",
    tags = [ "top-level-intermediate" ]
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('000_airbyte_dbt_1', '_airbyte_raw_property_history') }}
select
...
from {{ source('000_airbyte_dbt_1', '_airbyte_raw_property_history') }} as table_alias
-- property_history
where 1 = 1
